...
public class MongoListView : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        GUILayout.BeginHorizontal();
        {
            ...
            if (GUILayout.Button("Refresh"))
                PrepareData();
            if (GUILayout.Button("Get All Data From Cloud"))
        ...
        var selectedElements = !string.IsNullOrEmpty(query) ? GetFilterElement() : selectedCollection.elements;
        foreach (var collection in selectedElements)
        {
            ...
            var selectedDocumentOfElements = PreviousTextInput.FirstOrDefault(e => e.Key == collection["_id"].ToString()).Value;
            int collectionDataIndex = 0;
            foreach (var collectionElement in collection.Values)
            {
                if (collectionElement != collection["_id"])
                    selectedDocumentOfElements[collectionDataIndex] = EditorGUILayout.TextField(selectedDocumentOfElements[collectionDataIndex]);
                collectionDataIndex++;
            }
    ...
    private List<BsonDocument> GetFilterElement()
    {
        var matchingKeys = PreviousTextInput
            .Where(pair => pair.Value.Any(value => value.StartsWith(query)))
            .Select(pair => pair.Key)
            .ToList();

        var matchingElements = new List<BsonDocument>();

        foreach (var key in matchingKeys)
            matchingElements.AddRange(selectedCollection.elements.FindAll(e => e["_id"].ToString() == key));
